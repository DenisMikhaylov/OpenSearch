...
2. Объедините их в дашборд **Cluster Health Dashboard**.
...
|----------|---------------------|
| Отказ data-узла | Кластер остаётся доступным (status yellow/green) |
| Отказ cluster manager узла | Кластер остаётся управляемым, выбирается новый master |
| Отказ search-узла | Поиск продолжает работать через другие реплики |
| Распределение запросов | Нагрузка распределена между несколькими узлами |
| Восстановление после сбоя | Кластер автоматически восстанавливается в green |
...
|----------|-------------------|
| Классетр стал red после отказа узла | Проверьте, что у индекса есть реплики. Если нет — добавьте `number_of_replicas: 1`. |
| Поиск работает медленно после отказа | Дождитесь перераспределения шардов. Используйте `_cluster/reroute` для ускорения. |
| Cluster manager не выбирается | Убедитесь, что минимум 2 из 3 cluster manager узлов работают. |
| Запросы не распределяются равномерно | Проверьте настройки шардов и реплик. Используйте `_cat/allocation` для анализа. |
...
1. Проверили **отказоустойчивость** кластера при отказе data, cluster manager и search узлов.
2. Убедились, что **кворум из 3 узлов** обеспечивает управляемость при отказе одного узла.
3. Проверили **распределение поисковых запросов** между репликами.
4. Проанализировали поведение кластера при различных сценариях сбоев.
5. Научились использовать API для мониторинга состояния кластера.
